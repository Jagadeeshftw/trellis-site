  ...
  return (
    <Container className="border-divide relative flex min-h-60 flex-col items-center justify-center overflow-hidden border-x px-4 py-4 md:min-h-120">
      ...
      <SectionHeading className="relative z-10 text-center lg:text-6xl">
        Come and plant something. <br /> It will still be there.
      </SectionHeading>
      <Button as="a" href={site.openUrl} className="relative z-20 mt-4">
        Open on mobile
      </Button>
      <p className="font-mono relative z-20 mt-3 text-xs text-gray-600 dark:text-neutral-400">
        {site.realm}
      </p>
    </Container>
  );
...
export const CTAOrbit: React.FC<CTAOrbitProps> = ({
  ...
  const logos: SvgComponent[] = Object.values(propIcons);
